This directory contains the DemocoinQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the Democoin UI.
...
ClientModel is used by the main application `democoingui` and several models like `peertablemodel`.
...
Used to process BIP21 and BIP70 (see https://github.com/democoin/democoin/pull/11622) payment URI / requests. Also handles URI based application switching (e.g. when following a democoin:... link from a browser).
...
* UI elements like DemocoinAmountField, which inherit from QWidget.
* `democoinstrings.cpp`: automatically generated
* `democoinunits.(h/cpp)`: DMC / mDMC / etc handling
* `callback.h`
...
* don't change `local/democoin_en.ts`; this happens [automatically](/doc/translation_process.md#writing-code-with-translations)
...
10. Start debugging with Qt Creator (you might need to the executable to "democoin-qt" under "Run", which is where you can also add command line arguments)
