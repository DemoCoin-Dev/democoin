...
const char * const BITCOIN_PID_FILENAME = "democoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "democoind -foo=bar
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Democoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Democoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Democoin Core developers";
    }
