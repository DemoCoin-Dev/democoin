#!/usr/bin/env bash
# Copyright (c) 2016 The Democoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

###   This script attempts to download the signature file SHA256SUMS.asc from
###   democoincore.org and democoin.org and compares them.
...
WORKINGDIR="/tmp/democoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://democoincore.org"
HOST2="https://democoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="democoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'democoin-',
...
if [ $? -ne 0 ]; then
   echo "democoin.org failed to provide signature file, but democoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "democoin.org and democoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Democoin Core binary release signing key installed?"
   fi
